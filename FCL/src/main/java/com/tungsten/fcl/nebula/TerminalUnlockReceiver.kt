 ...
 *
 * 第一步（申请挑战码，写入公共目录）：
 *   adb shell am broadcast -n com.nebula.launcher/com.tungsten.fcl.nebula.TerminalUnlockReceiver \
 *     -a com.nebula.launcher.REQ_TERMINAL_CHALLENGE
 *   挑战码文件位置（按权限自动选择）：
 ...
 * 第二步（携带挑战码和应答码解锁）：
 *   adb shell am broadcast -n com.nebula.launcher/com.tungsten.fcl.nebula.TerminalUnlockReceiver \
 *     -a com.nebula.launcher.ENABLE_TERMINAL \
 *     --es c "挑战码" --es r "应答码"
 *
 * 挑战码 30 分钟后过期。
 */
class TerminalUnlockReceiver : BroadcastReceiver() {
    ...
    override fun onReceive(context: Context, intent: Intent) {
        ...
        if (action == "com.nebula.launcher.REQ_TERMINAL_CHALLENGE") {
            val challenge = randomHex()
            val prefs = context.getSharedPreferences("nebula_settings", Context.MODE_PRIVATE)
            prefs.edit()
        ...
        if (action == "com.nebula.launcher.ENABLE_TERMINAL") {
