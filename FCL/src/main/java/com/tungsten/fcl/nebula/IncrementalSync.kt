...
class IncrementalSync(
    ...
    private val hasher: NebulaHasher
) {
    ...
    suspend fun sync(
        ...
        versionDir: File,
        threadCount: Int = 8,
        onProgress: (NebulaProgress) -> Unit
        ...
        for (t in tasks) {
            ...
            if (local.exists() && local.length() == t.file.size &&
                (t.file.sha256.isBlank() || hasher.sha256(local).equals(t.file.sha256, true))
            ) {
                skipped++
            } else {
                toDownload.add(t)
        ...
        onProgress(NebulaProgress(0, "增量同步：需下载 $total，已最新 $skipped"))
...
data class NebulaProgress(val percent: Int, val message: String)
